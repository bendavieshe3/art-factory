...
class TemplateTestRunner:
    ...
    def __init__(self, verbosity=2):
        ...
        self.results = {"passed": 0, "failed": 0, "errors": 0, "skipped": 0, "warnings": []}
    ...
    def _generate_report(self):
        ...
        total_tests = sum([self.results["passed"], self.results["failed"], self.results["errors"], self.results["skipped"]])
...
class TemplateLintCommand:
    ...
    def run(args=None):
        ...
        parser = argparse.ArgumentParser(description="Run template tests for AI Art Factory")
        parser.add_argument(
            "--category", choices=["framework", "validation", "all"], default="all", help="Test category to run"
        )
        ...
        if args.category == "all":
            ...
            runner = unittest.TextTestRunner(verbosity=args.verbosity, failfast=args.fail_fast)
            runner.run(suite)
...
class Command:
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--fix", action="store_true", help="Attempt to fix issues automatically (where possible)")
