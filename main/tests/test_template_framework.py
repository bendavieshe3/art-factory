...
class TemplateInheritanceTestCase(TestCase):
    ...
    def test_base_template_structure(self):
        ...
        for block in required_blocks:
            self.assertIn(f"{{% block {block}", base_content, f"base.html missing required block: {block}")
    ...
    def test_layout_inheritance_chain(self):
        ...
        for layout, parent in inheritance_chain.items():
            ...
            self.assertIn(f"{{% extends '{parent}' %}}", template_content, f"{layout} should extend {parent}")
...
class ComponentConsistencyTestCase(TestCase):
    ...
    def test_modal_component_parameters(self):
        ...
        for template_path in self.templates_dir.rglob("*.html"):
            if template_path.is_file():
                content = template_path.read_text()
                modal_includes = re.findall(r"{%\s*include\s+'components/modals/.*?\.html'.*?%}", content)
                if modal_includes:
                    modal_usages.extend([(template_path.name, include) for include in modal_includes])
        ...
        for template_name, include_tag in modal_usages:
            if "confirmation_modal.html" in include_tag:
                ...
                self.assertIn("modal_id=", include_tag, f"{template_name}: confirmation_modal requires modal_id")
                self.assertIn("title=", include_tag, f"{template_name}: confirmation_modal requires title")
                self.assertIn("message=", include_tag, f"{template_name}: confirmation_modal requires message")
    ...
    def test_javascript_component_loading_order(self):
        ...
        js_includes = re.findall(r"{%\s*include\s+'components/js/.*?\.html'.*?%}", order_content)

        # Verify key components are present in correct order
        # First should be form management (foundation)
        self.assertTrue(
            any("order_form_management.html" in include for include in js_includes),
            "order_form_management.html should be included",
        )

        # Last should be main init (orchestrator)
        self.assertTrue(
            any("order_main_init.html" in include for include in js_includes), "order_main_init.html should be included"
        )

        # Form management should come before main init
        form_mgmt_idx = next((i for i, include in enumerate(js_includes) if "order_form_management.html" in include), -1)
        main_init_idx = next((i for i, include in enumerate(js_includes) if "order_main_init.html" in include), -1)

        if form_mgmt_idx >= 0 and main_init_idx >= 0:
            self.assertLess(form_mgmt_idx, main_init_idx, "Form management should load before main init")
...
class TemplateRenderingTestCase(TestCase):
    ...
    def test_layout_rendering_with_context(self):
        ...
        contexts = [
            {"page_title": "Test Page", "card_title": "Test Card", "card_icon": "gear", "card_class": "border-primary"},
            {
    ...
    def test_responsive_layout_classes(self):
        ...
        for layout_name in layouts:
            ...
            self.assertTrue(len(containers) > 0, f"{layout_name} should have responsive container")
            ...
            if layout_name == "sidebar_layout.html":
                ...
                self.assertTrue(len(cols) >= 2, "sidebar_layout should have responsive columns")
...
class TemplateAccessibilityTestCase(TestCase):
    ...
    def test_aria_labels_in_components(self):
        ...
        self.assertIn("aria-label", pagination_content, "Pagination should include aria-label")
        self.assertIn("page-item active", pagination_content, "Pagination should mark current page")

    def test_form_labels(self):
        """Test form components have proper labels."""
        form_templates = ["components/forms/project_form.html", "main/order.html"]
        ...
        for template_name in form_templates:
            try:
                ...
                self.assertIn("<label", content, f"{template_name} should include form labels")

                # Check for for attributes
                if "for=" in content:
                    self.assertIn('for="', content, f"{template_name} labels should have for attributes")
            except Exception:
                ...
                if "<img" in content and "components/" not in str(template_path):
                    ...
                    for img_tag in img_tags:
                        if "alt=" not in img_tag:
                            ...
                            if "{{" not in img_tag:
                                self.fail(f"{template_path.name} has img without alt: {img_tag}")
            ...
            if template_path.is_file():
                ...
                if "<form" in content and 'method="post"' in content.lower():
                    form_blocks = re.findall(
                        r'<form[^>]*method=["\']post["\'][^>]*>.*?</form>', content, re.DOTALL | re.IGNORECASE
                    )

                    for form in form_blocks:
                        if "{% csrf_token %}" not in form:
                            self.fail(f"{template_path.name} has POST form without CSRF token")
                            ...
                            if "|escapejs" not in match and "|json_script" not in match:
                                ...
                                if "components/js/" in str(template_path):
                                    ...
                                    if "|safe" in match and "_json" in match:
                                        continue
                                self.fail(f"{template_path.name} has potentially unsafe JavaScript: {match[:50]}...")
...
class ComponentDocumentationTestCase(TestCase):
    ...
    def test_all_components_documented(self):
        ...
        for component in component_files:
            self.assertIn(
                component.replace("\\", "/"), doc_content, f"Component {component} not documented"  # Handle Windows paths
            )
    ...
    def test_component_parameter_documentation(self):
        ...
        component_sections = re.findall(r"####\s+`([^`]+)`(.*?)(?=####|$)", doc_content, re.DOTALL)
        ...
        for component_path, section_content in component_sections:
            if "**Parameters**:" in section_content:
                ...
                if not has_none:
                    ...
                    self.assertTrue(
                        has_required or has_optional, f"{component_path} should document parameter status (required/optional)"
                    )
