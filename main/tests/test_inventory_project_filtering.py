...
class InventoryProjectFilteringTest(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create(
            name="Test Inventory Project",
            description="Testing inventory project filtering",
            status="active"
        )
    ...
    def test_inventory_shows_all_batch_products_when_filtered_by_project(self):
        ...
        self.assertEqual(len(all_products_json), 6, 
                        "Inventory should show all 6 products when not filtered")
        ...
        self.assertEqual(len(filtered_products_json), 4, 
                        "Inventory should show all 4 batch products when filtered by project")
        
        # Verify all our batch products are in the filtered results
        filtered_product_titles = [p['title'] for p in filtered_products_json]
        for i in range(4):
            self.assertIn(f"Batch Product {i}", filtered_product_titles,
                         f"Batch Product {i} should be in filtered inventory")
        
        # Verify other project's products are not included
        for i in range(2):
            self.assertNotIn(f"Other Product {i}", filtered_product_titles,
                           f"Other Product {i} should not be in filtered inventory")
    ...
    def test_inventory_filtering_with_multiple_batch_orders_in_same_project(self):
        ...
        self.assertEqual(len(filtered_products_json), 5,
                        "Inventory should show all products from both batch orders")
        
        # Verify all products are present
        filtered_product_titles = [p['title'] for p in filtered_products_json]
        
        for i in range(3):
            self.assertIn(f"First Batch Product {i}", filtered_product_titles,
                         f"First Batch Product {i} should be in filtered inventory")
        
        for i in range(2):
            self.assertIn(f"Second Batch Product {i}", filtered_product_titles,
    ...
    def test_inventory_pagination_with_project_filtering(self):
        ...
        self.assertEqual(len(page1_products), 20,
                        "First page should show 20 products")
        
        # Test second page
        response = self.client.get(reverse('main:inventory'), 
                                 {'project': self.project.id, 'page': 2})
        self.assertEqual(response.status_code, 200)
        
        # Should show remaining 5 products
        page2_products = json.loads(response.context['products_json'])
        self.assertEqual(len(page2_products), 5,
                        "Second page should show remaining 5 products")