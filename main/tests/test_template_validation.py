    ...
    def test_template_block_matching(self):
        ...
        for template_path in self._get_all_templates():
            ...
            if len(block_starts) != len(block_ends):
                errors.append(
                    f"{template_path.name}: Mismatched blocks - " f"{len(block_starts)} starts, {len(block_ends)} ends"
                )
    ...
    def test_template_tag_matching(self):
        ...
        for template_path in self._get_all_templates():
            ...
            for start_tag, end_tag in tag_pairs:
                ...
                if starts != ends:
                    errors.append(f"{template_path.name}: Mismatched {start_tag}/{end_tag} - " f"{starts} starts, {ends} ends")
...
class TemplateConsistencyTestCase(TestCase):
    ...
    def test_consistent_layout_usage(self):
        ...
        for template_name, expected_layout in layout_expectations.items():
            ...
            if template_path.exists():
                content = template_path.read_text()
                self.assertIn(f"{{% extends '{expected_layout}' %}}", content, f"{template_name} should use {expected_layout}")
    ...
    def test_consistent_empty_states(self):
        ...
        templates_with_lists = ["projects.html", "all_projects.html", "production.html", "project_detail.html"]
        ...
        for template_name in templates_with_lists:
            ...
            if template_path.exists():
                ...
                self.assertTrue(
                    ...
                    or "{% else %}" in content
                    or "{% empty %}" in content
                    or "No " in content,  # Allow "No products yet" style messages
    ...
    def test_static_file_references(self):
        ...
        for template_path in self._get_all_templates():
            ...
            for ref in static_refs:
                ...
                if "{{" in ref or "{%" in ref:
                    errors.append(f"{template_path.name}: Invalid static reference: {ref}")
        ...
        for template_path in self._get_all_templates():
            ...
            for ref in url_refs:
                ...
                if ":" not in ref and not ref.startswith("admin:"):
                    errors.append(f"{template_path.name}: URL without namespace: {ref}")
...
class TemplatePerformanceTestCase(TestCase):
    ...
    def test_no_excessive_includes(self):
        ...
        for template_path in self._get_all_templates():
            ...
            if len(includes) > 10:
                warnings.append(f"{template_path.name}: High number of includes ({len(includes)})")
    ...
    def test_no_deep_nesting(self):
        ...
        for template_path in self._get_all_templates():
            ...
            if max_depth > 15:
                warnings.append(f"{template_path.name}: Deep nesting detected (depth: {max_depth})")
