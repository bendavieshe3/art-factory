...
class AsyncProcessingTestCase(TestCase):
    ...
    def test_monitor_orders_command(self):
        ...
        OrderItem.objects.create(
            order=order1, prompt=order1.prompt, parameters=self.flux_machine.default_parameters, status="pending"
        )
    ...
    def test_process_pending_orders_dry_run(self):
        ...
        item = OrderItem.objects.create(
            order=order, prompt=order.prompt, parameters=self.flux_machine.default_parameters, status="pending"
        )
    ...
    def test_process_pending_orders_execution(self, mock_process):
        ...
        log_entries = LogEntry.objects.filter(
            logger_name="management.process_pending_orders", extra_data__event_type="retry_processing"
        )
    ...
    def test_process_pending_orders_with_failed_retry(self):
        ...
        out = StringIO()
        call_command("process_pending_orders", "--retry-failed", "--dry-run", "--max-age", "5", stdout=out)
        output = out.getvalue()
    ...
    def test_pending_order_age_calculation(self):
        ...
        recent_item = OrderItem.objects.create(
            order=recent_order, prompt=recent_order.prompt, parameters=self.flux_machine.default_parameters, status="pending"
        )
        ...
        old_item = OrderItem.objects.create(
            order=old_order, prompt=old_order.prompt, parameters=self.flux_machine.default_parameters, status="pending"
        )
        ...
        old_items = OrderItem.objects.filter(status="pending", created_at__lt=cutoff_time)
    ...
    def test_retry_mechanism_preserves_order_data(self):
        ...
        item = OrderItem.objects.create(order=order, prompt=order.prompt, parameters=merged_params, status="pending")
