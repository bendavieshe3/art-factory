...
class BatchProjectAssignmentTest(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create(
            ...
            description="Testing batch project assignment",
            status="active"
        )
    ...
    def test_batch_products_inherit_project_from_order(self):
        ...
        for i, product in enumerate(products):
            ...
            self.assertEqual(expected_project, self.project, 
                           f"Product {i} should be linked to project through order relationship")
            
        # Verify we can find products by project through the order relationship
        project_product_ids = Product.objects.filter(
            order_item__order__project=self.project
        ).values_list('id', flat=True)
        
        created_product_ids = [p.id for p in products]
        
        for product_id in created_product_ids:
            self.assertIn(product_id, project_product_ids, 
                         "All batch products should be findable through project relationship")
    ...
    def test_project_recent_products_includes_all_batch_products(self):
        ...
        for i, product in enumerate(products):
            self.assertIn(product.id, recent_product_ids,
                         f"Batch product {i} should be in project's recent products")
        
        # Verify the project count is correct
        self.assertEqual(self.project.product_count, 3, 
                        "Project should count all 3 batch products")
    ...
    def test_project_counts_with_multiple_batches(self):
        ...
        all_project_products = Product.objects.filter(
            order_item__order__project=self.project
        )
        self.assertEqual(all_project_products.count(), 5, 
                        "Should be able to find all 5 products through project relationship")