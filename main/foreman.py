...
class Foreman:
    ...
    def run(self):
        ...
        try:
            ...
            LogEntry.objects.create(
                ...
                logger_name="foreman",
                extra_data={"event_type": "foreman_started", "process_id": self.process_id},
            )
            ...
            LogEntry.objects.create(
                ...
                logger_name="foreman",
                extra_data={"event_type": "foreman_error", "error": str(e)},
            )
    ...
    def handle_stalled_workers(self):
        ...
        stalled_workers = Worker.objects.filter(last_heartbeat__lt=timezone.now() - timedelta(minutes=3))
            ...
            if self.process_exists(worker.process_id):
                try:
                    ...
                    LogEntry.objects.create(
                        ...
                        logger_name="foreman",
                        extra_data={"event_type": "worker_killed", "worker_id": worker.id, "process_id": worker.process_id},
                    )
    ...
    def reassign_worker_items(self, worker):
        ...
        assigned_items = OrderItem.objects.filter(assigned_worker=worker, status__in=["assigned", "processing"])
        ...
        if count > 0:
            ...
            LogEntry.objects.create(
                ...
                logger_name="foreman",
                extra_data={"event_type": "work_reassigned", "worker_id": worker.id, "items_reassigned": count},
            )

    def reassign_orphaned_work(self):
        """Find and reassign work that has no assigned worker."""
        orphaned_items = OrderItem.objects.filter(status="assigned", assigned_worker__isnull=True)
        ...
        if count > 0:
            ...
            LogEntry.objects.create(
                ...
                logger_name="foreman",
                extra_data={"event_type": "orphaned_work_reassigned", "items_count": count},
            )
        ...
        try:
            ...
            if pending_items > 0 and active_workers == 0:
                ...
                LogEntry.objects.create(
                    ...
                    logger_name="foreman",
                    extra_data={"event_type": "no_workers_alert", "pending_items": pending_items},
                )
                ...
                try:
                    ...
                    LogEntry.objects.create(
                        ...
                        logger_name="foreman",
                        extra_data={"event_type": "worker_spawned_by_foreman", "pending_items": pending_items},
                    )
                except Exception as e:
