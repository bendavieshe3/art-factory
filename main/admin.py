...
class ProductAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'description', 'product_type', 'is_favorite')
        }),
        ('File Information', {
            'fields': ('file_path', 'file_url', 'file_size', 'file_format', 'width', 'height', 'duration')
        }),
        ('Generation Details', {
            'fields': ('prompt', 'parameters', 'seed', 'provider', 'model_name', 'provider_id')
        }),
        ('Organization', {
            'fields': ('tags',)
        }),
...
class OrderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Generation Request', {
            'fields': ('prompt', 'base_parameters', 'factory_machine_name', 'provider')
        }),
        ('Batch Settings', {
            'fields': ('quantity', 'expand_parameters')
        }),
        ('Organization', {
            'fields': ('project_name', 'tags')
        }),
        ('Status', {
            'fields': ('completion_percentage', 'created_at', 'updated_at', 'completed_at'),
            'classes': ('collapse',)
        }),
    )
...
class FactoryMachineDefinitionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Provider Details', {
            'fields': ('provider', 'model_family', 'modality')
        }),
        ('Parameters', {
            'fields': ('parameter_schema', 'default_parameters')
        }),
        ('Operational Settings', {
            'fields': ('max_concurrent_jobs', 'estimated_duration', 'cost_per_operation')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
