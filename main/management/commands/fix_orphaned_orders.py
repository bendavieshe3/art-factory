...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.HTTP_INFO("Fixing Orphaned Orders"))
        ...
        for order in orphaned_orders:
            ...
            if not dry_run:
                ...
                try:
                    machine_def = FactoryMachineDefinition.objects.get(name=order.factory_machine_name)
                    ...
                    for i in range(order.quantity):
                        item = OrderItem.objects.create(
                            order=order, prompt=order.prompt, parameters=merged_params, status="pending"
                        )
                        created_items.append(item)

                    self.stdout.write(self.style.SUCCESS(f"   ✅ Created {len(created_items)} OrderItems"))

                    # Update order status if it's still pending
                    if order.status == "pending" and created_items:
                        order.status = "processing"
                        order.save()
                        self.stdout.write("   📊 Updated order status to processing")

                except FactoryMachineDefinition.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"   ❌ Machine definition not found: {order.factory_machine_name}"))
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"   ❌ Error creating items: {e}"))
            else:
                self.stdout.write(self.style.WARNING(f"   🔸 Would create {order.quantity} OrderItems (dry run)"))

        if dry_run:
            self.stdout.write(self.style.WARNING("\n⚠️  Dry run complete - no changes made"))
            self.stdout.write("Run without --dry-run to fix these orders")
