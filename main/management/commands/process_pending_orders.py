...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=5,
            help='Maximum age in minutes for orders to retry (default: 5)'
        )
        parser.add_argument(
            '--batch-size',
            type=int,
            default=5,
            help='Maximum number of orders to process at once (default: 5)'
        )
        parser.add_argument(
            '--retry-failed',
            action='store_true',
            help='Also retry failed orders (not just pending)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if retry_failed:
            failed_items = OrderItem.objects.filter(
                status='failed', 
            ...
            all_items = OrderItem.objects.filter(
                Q(status='pending', created_at__lt=cutoff_time) |
                Q(status='failed', updated_at__lt=cutoff_time)
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Processing initiated: {processed_count}/{len(all_items)} orders "
                    f"are now processing or completed"
                )
            )
