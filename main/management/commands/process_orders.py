...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["order_item_id"]:
            ...
            if result.success:
                self.stdout.write(self.style.SUCCESS(f'✅ Order item {options["order_item_id"]} completed successfully'))
                if result.product:
                    self.stdout.write(f"   Product created: {result.product.id}")
            else:
                self.stdout.write(self.style.ERROR(f'❌ Order item {options["order_item_id"]} failed: {result.error}'))
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Error processing orders: {e}"))
