...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--details',
            action='store_true',
            help='Show detailed information about each order'
        )
        parser.add_argument(
            '--pending-only',
            action='store_true',
            help='Only show pending/stuck orders'
        )
        parser.add_argument(
            '--recent-hours',
            type=int,
            default=24,
            help='Show orders from the last N hours (default: 24)'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.HTTP_INFO(f"Order Processing Status (last {recent_hours} hours)")
        )
        ...
        if pending_only:
            orders = Order.objects.filter(
                ...
                orderitem__status__in=['pending', 'processing']
        ...
        if pending_items.exists():
            ...
            for item in pending_items.order_by('created_at'):
                ...
                self.stdout.write(
                    f"  - Item {item.id} (Order {item.order.id}): "
                    f"pending for {age} - {item.order.factory_machine_name}"
                )
        ...
        recent_logs = LogEntry.objects.filter(
            ...
            extra_data__event_type__in=['processing_started', 'item_completed', 'item_failed']
        ...
        if recent_logs.exists():
            ...
            for log in recent_logs:
                ...
                self.stdout.write(
                    f"  {emoji} {log.timestamp.strftime('%H:%M:%S')}: {log.message}"
                )
        ...
        if status_counts.get('pending', 0) > 0:
            self.stdout.write(
                f"  - Run 'python manage.py process_pending_orders' to retry stuck orders"
            )
        
        if status_counts.get('failed', 0) > 0:
            self.stdout.write(
                f"  - Run 'python manage.py process_pending_orders --retry-failed' to retry failed orders"
            )
        
        if status_counts.get('completed', 0) > 0:
            self.stdout.write(
                f"  - Check /inventory/ to view {status_counts['completed']} generated products"
            )