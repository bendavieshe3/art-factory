...
from main.models import Order, OrderItem, Product, LogEntry, FactoryMachineDefinition
import json
    ...
    def show_system_status(self):
        ...
        if recent_logs:
            for log in recent_logs:
                ...
                self.stdout.write(f"  {log.timestamp.strftime('%m/%d %H:%M')} "
                                f"{level_color(log.level)}: {log.message}")
        else:
