...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.HTTP_INFO("Worker System Status")
        )
        ...
        if workers:
            ...
            for worker in workers:
                ...
                self.stdout.write(
                    f"  {status_emoji} {worker.name} (PID: {worker.process_id}){stall_indicator}"
                )
        ...
        recent_items = OrderItem.objects.exclude(
            status='pending'
        ...
        if recent_items.exists():
            ...
            for item in recent_items:
                ...
                self.stdout.write(
                    f"  {status_emoji} Item {item.id}: {item.status}{worker_info} - {age} ago"
                )
