...
class ProductViewerModalTest(TestCase):
    ...
    def test_recent_orders_api_data_structure(self):
        ...
        OrderItem.objects.create(
            order=self.order, prompt="Test item 1", status="completed", parameters={"width": 512, "height": 512}
        )
        OrderItem.objects.create(
            order=self.order, prompt="Test item 2", status="failed", parameters={"width": 512, "height": 512}
        )
